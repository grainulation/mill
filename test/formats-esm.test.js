...
async function run() {
  ...
  assert.ok(files.length > 0, 'Expected at least one format module in lib/formats/');
  ...
  for (const file of files) {
    ...
    try {
      ...
      assert.strictEqual(typeof mod.name, 'string', `${formatName}.name should be a string`);
      assert.ok(mod.name.length > 0, `${formatName}.name should not be empty`);
      assert.strictEqual(typeof mod.extension, 'string', `${formatName}.extension should be a string`);
      assert.ok(mod.extension.startsWith('.'), `${formatName}.extension should start with "."`);
      assert.strictEqual(typeof mod.convert, 'function', `${formatName}.convert should be a function`);

      // Check optional exports if present
      if (mod.mimeType) {
        assert.strictEqual(typeof mod.mimeType, 'string', `${formatName}.mimeType should be a string`);
      }
      if (mod.description) {
        assert.strictEqual(typeof mod.description, 'string', `${formatName}.description should be a string`);
      }

      console.log(`  PASS  ${formatName}: name="${mod.name}", ext="${mod.extension}"`);
      passed++;
    } catch (err) {
