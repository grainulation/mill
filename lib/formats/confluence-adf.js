...
export const description = 'Confluence ADF (Atlassian Document Format) for direct API upload';
...
export function convert(compilation) {
  ...
  content.push(
    table(
      [tableRow([
        tableHeader('Metric'), tableHeader('Count'),
      ])],
      [
        tableRow([tableCell('Total claims'), tableCell(String(claims.length))]),
        tableRow([tableCell('Active'), tableCell(String(active))]),
        ...(conflicts.length > 0 ? [tableRow([tableCell('Conflicts'), tableCell(String(conflicts.length))])] : []),
      ],
    ),
  );
  ...
  for (const t of sortedTypes) {
    ...
    if (t === 'risk' && group.length > 0) {
      content.push(panel('warning', group.length + ' risk(s) identified -- review before proceeding'));
    } else if (t === 'constraint' && group.length > 0) {
      content.push(panel('error', group.length + ' hard constraint(s) -- these are non-negotiable'));
    }
    ...
    content.push(
      table(
        ...
        group.map(c => {
          ...
          const tier = typeof c.evidence === 'string' ? c.evidence : (c.evidence?.tier || c.evidence_tier || '');
    ...
    if (group.length > 10) {
      content.push(paragraph([
        text(`Showing all ${group.length} ${t} claims. Consider using Confluence page filtering for large datasets.`, [mark('em')]),
      ]));
    }
  }
  ...
  if (conflicts.length > 0) {
    ...
    const items = conflicts.map(c => {
      const ids = Array.isArray(c.ids) ? c.ids.join(' vs ') : (c.between || 'unknown');
  ...
  content.push(rule());
  content.push(paragraph([
    text(`Certificate: ${certificate.claim_count || claims.length} claims | sha256:${(certificate.sha256 || 'unknown').slice(0, 16)}`, [mark('code')]),
  ]));
